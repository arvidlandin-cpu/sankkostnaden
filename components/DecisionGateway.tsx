...
const config={
  ...
  el:{title:'Vad vill du göra nu?',intro:'Välj om du vill jämföra flera elavtal, gå direkt till ett elbolag eller börja med avtalsformen.',direct:'Hitta relevanta elavtal',help:'Hjälp mig välja avtalsform',helpHref:'/elavtal/vilket-elavtal-passar-mig/',fallback:'/elavtal/jamfor-elavtal/'},
    ...
    <div className='gatewayPaths gatewayPathsSimple'>
      {onePartner?
        ...
        <a className='gatewayPath gatewayDirect' href={directHref}>
          <span className='gatewayIcon'><Gauge size={20}/></span><div><small>JÄMFÖR NU</small><strong>{directLabel}</strong><p>{active.length>1?(samePage?(category==='mobil'?'Svara på två frågor och få tre relevanta partneralternativ.':category==='bredband'?'Svara på två frågor om adress och teknik och få en bättre startpunkt.':category==='el'?'Välj jämförelseväg och få en relevant kortlista.':category==='ekonomi'?'Svara på två frågor och få en relevant kortlista av jämförelsetjänster.':'Se bara partners som matchar rätt typ av skydd.'):'Gå vidare till jämförelsesidan där de relevanta alternativen samlas tydligt.'):'Gå vidare till jämförelsen och se vad som är relevant för ditt behov.'}</p></div><ArrowRight size={20}/>
        </a>}
